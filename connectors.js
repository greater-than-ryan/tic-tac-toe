...
function drawBoard(board) {
  ...
  for (let rowIndex = 0; rowIndex < 3; rowIndex++) {
    for (let columnIndex = 0; columnIndex < 3; columnIndex++) {
      ...
      const cellText = board[rowIndex][columnIndex] === "nought" ? "⭕︎" : "✖";
      document.getElementById(
        `row-${rowIndex}-column-${columnIndex}`
      ).innerText = cellText;
    }
...
function positionClick(rowIndex, columnIndex, event) {
  ...
  if (winner) {
    ...
    if (winner === "noughts") {
      winnerName.innerText = document.getElementById("player1name").value;
    }

    if (winner === "crosses") {
      winnerName.innerText = document.getElementById("player2name").value;
    }
